...
const T_CRO_ADDRESS = "0xeAdf7c01DA7E93FdB5f16B0aa9ee85f978e89E95";
const RPC_URL = "https://evm.cronos.org";
const CRONOS_CHAIN_ID = 25;

// Constants for Sanity Checks (CRO has 18 decimals, so Rate is scaled by 1e(18+18-8) = 1e28)
const RATE_SANITY_MIN = BigInt(10) ** BigInt(26); // 0.01 (Base 1e28)
const RATE_SANITY_MAX = BigInt(10) ** BigInt(29); // 10.0
...
export class TectonicCroAdapter {
    ...
    constructor() {
        ...
        this.tToken = new ethers.Contract(T_CRO_ADDRESS, ABI, this.provider);
    }
    ...
    async fetchAndSync(): Promise<void> {
        try {
            ...
            try {
                ...
                console.warn(`[Tectonic-CRO] exchangeRateCurrent staticCall failed (${e.message}), falling back to stored.`);
                exchangeRate = await this.tToken.exchangeRateStored();
            }

            // Sanity Check Rate
            if (exchangeRate < RATE_SANITY_MIN || exchangeRate > RATE_SANITY_MAX) {
                throw new Error(`[Tectonic-CRO] Abnormal Exchange Rate detected: ${exchangeRate}. Aborting Sync.`);
            }
            ...
            const apyPercent = Number(apyMantissa) / 1e16; // 1e18 scale -> percent
            ...
            await YieldSource.findOneAndUpdate(
                { id: "TECTONIC_CRO" },
                {
                    ...
                    vaultAddress: T_CRO_ADDRESS,
                    type: "LENDING",
                    status: status,
                    executable: false, // For now, we only execute on tUSDC manually
                    lastHarvestedAt: now,
            ...
            console.log(`✅ [Tectonic-CRO] Synced. Status: ${status}, APY: ${apyPercent.toFixed(2)}%`);
        } catch (error: any) {
            console.error("❌ [Tectonic-CRO] Sync Failed:", error.message);
        }
    }
}
