...
contract Tonic {
    ...
    string public constant name = "Tectonic Governance Token";

    /// @notice EIP-20 token symbol for this token
    string public constant symbol = "TONIC";

    /// @notice EIP-20 token decimals for this token
    uint8 public constant decimals = 18;

    /// @notice Total number of tokens in circulation
    uint public constant totalSupply = 500000000000000e18; // 500,000,000,000,000 Tonic, 500 Trillion
    ...
    /**
     * @notice Construct a new Tonic token
    ...
    function approve(address spender, uint rawAmount) external returns (bool) {
        ...
        if (rawAmount == uint(-1)) {
            ...
            amount = safe128(rawAmount, "Tonic::approve: amount exceeds 128 bits");
        }
    ...
    function transfer(address dst, uint rawAmount) external returns (bool) {
        uint128 amount = safe128(rawAmount, "Tonic::transfer: amount exceeds 128 bits");
    ...
    function transferFrom(address src, address dst, uint rawAmount) external returns (bool) {
        ...
        uint128 amount = safe128(rawAmount, "Tonic::approve: amount exceeds 128 bits");

        if (spender != src && spenderAllowance != uint128(-1)) {
            uint128 newAllowance = sub128(spenderAllowance, amount, "Tonic::transferFrom: transfer amount exceeds spender allowance");
    ...
    function getPriorVotes(address account, uint blockNumber) public view returns (uint128) {
        require(blockNumber < block.number, "Tonic::getPriorVotes: not yet determined");
    ...
    function _transferTokens(address src, address dst, uint128 amount) internal {
        require(src != address(0), "Tonic::_transferTokens: cannot transfer from the zero address");
        require(dst != address(0), "Tonic::_transferTokens: cannot transfer to the zero address");

        balances[src] = sub128(balances[src], amount, "Tonic::_transferTokens: transfer amount exceeds balance");
        balances[dst] = add128(balances[dst], amount, "Tonic::_transferTokens: transfer amount overflows");
    ...
    function _moveDelegates(address srcRep, address dstRep, uint128 amount) internal {
        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                ...
                uint128 srcRepNew = sub128(srcRepOld, amount, "Tonic::_moveVotes: vote amount underflows");
            ...
            if (dstRep != address(0)) {
                ...
                uint128 dstRepNew = add128(dstRepOld, amount, "Tonic::_moveVotes: vote amount overflows");
    ...
    function _writeCheckpoint(address delegatee, uint32 nCheckpoints, uint128 oldVotes, uint128 newVotes) internal {
      uint32 blockNumber = safe32(block.number, "Tonic::_writeCheckpoint: block number exceeds 32 bits");
